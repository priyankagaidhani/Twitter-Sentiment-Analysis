...
def load_data():
    consumer_key = 'JBtEtLNix0QzvrO333Ng4upMj'
    consumer_secret = 'i9USn73ptELhxzCAzq2L4yhu8iCaMqhhXXaxeO5uGaC9XGtIUS'
    access_token = '1639251234132467712-T0U7kshqvnK02ulNVqpIwZKmajIJeP'
    access_token_secret = 'JuhM4dU7OoYbb2OO1P61Bnsc8jEoLTUyQZThnyKHoeEPP'
    ...
    # Define search keyword and number of tweets to retrieve
    search_word = '#IPL2023'
    tweet_count = 5


    # Search for tweets using the API object and Tweepy's Cursor() method
    tweets = tweepy.Cursor(api.search_tweets, q=search_word, tweet_mode='extended', count=tweet_count).items()
    # tweets = api.search(q="python", count=10)
    # Print the retrieved tweets
    # for tweet in tweets:
    # print(tweet.full_text)

    df = pd.DataFrame(columns=['tweet_id', 'tweet'])

    # Iterate over the retrieved tweets and add them to the dataframe
    # for tweet in tweets:
    # df = df.append({'tweet_id': tweet.id_str, 'text': tweet.full_text}, ignore_index=True)

    # Print the dataframe
    # print(df.head())

    for tweet in tweets:
        df = pd.concat([df, pd.DataFrame({'tweet_id': [tweet.id_str], 'tweet': [tweet.full_text]})])
...
sentenceWithoutStopword = [" ".join(sentence) for sentence in sentenceWithoutStopword]
